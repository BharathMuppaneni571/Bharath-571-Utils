...
import { Table, Copy, Trash2, ArrowRightLeft, AlertCircle } from 'lucide-react';
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-nexus-accent/20 flex items-center justify-center">
              <Table className="w-6 h-6 text-nexus-accent" />
            </div>
            <div>
              <h2 className="text-3xl font-bold glow-text tracking-tight">CSV ↔ JSON</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-4">
            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">
              {mode === 'csv2json' ? 'Input CSV' : 'Input JSON Array'}
            </label>
            <textarea
              ...
              placeholder={mode === 'csv2json' ? 'Paste CSV here (comma separated)...' : 'Paste JSON array here...'}
              value={input}
