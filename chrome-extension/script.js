...
window.handleAuth = async function(type) {
  const endpoint = `${API_BASE_URL}${type === 'login' ? '/api/auth/login' : '/api/auth/register'}`;
...
window.openModeSelector = async function() {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Auto-persist inputs within tiles
  document.querySelectorAll('.tile input, .tile textarea, .tile select').forEach(async el => {
    if (el.type === 'file' || el.id.includes('pass') || el.id.includes('auth')) return;
    const storageKey = 'toolbox_' + el.id;
    const data = await chrome.storage.local.get([storageKey]);
    const saved = data[storageKey];
    if (saved !== undefined) {
      if (el.type === 'checkbox') el.checked = (saved === 'true' || saved === true);
      else el.value = saved;
    }
    el.addEventListener('input', () => {
      chrome.storage.local.set({ [storageKey]: el.type === 'checkbox' ? el.checked : el.value });
    });
  });

  // Auto-persist CodeMirror editors
  setTimeout(async () => {
    if (window.cmEditors) {
      for (const key in window.cmEditors) {
        const storageKey = 'toolbox_cm_' + key;
        const data = await chrome.storage.local.get([storageKey]);
        const saved = data[storageKey];
        if (saved) window.cmEditors[key].setValue(saved);
        window.cmEditors[key].on('change', () => {
          chrome.storage.local.set({ [storageKey]: window.cmEditors[key].getValue() });
        });
      }
    }
  }, 1000);
